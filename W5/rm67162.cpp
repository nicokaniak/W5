              ...
              uint16_t color)
{
    ...
    uint16_t *color_p = (uint16_t *)heap_caps_malloc(pixelCount * 2, MALLOC_CAP_INTERNAL);
    if (!color_p) {
        // Try PSRAM if internal memory fails
        color_p = (uint16_t *)ps_malloc(pixelCount * 2);
    }
    if (!color_p) {
        Serial.println("lcd_fill: memory allocation failed!");
        return;
    }
    
    // Fill buffer with 16-bit color (memset only works for bytes!)
