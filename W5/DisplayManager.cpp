...
void DisplayManager::drawConfigMenu(uint8_t selectedIndex) {
  ...
  for (uint8_t i = 0; i < count; i++) {
    ...
    // Track the widest/tallest label so the selection bracket fits all options.
    uint16_t lw, lh;
    text7segBounds(label, 3, &lw, &lh);
    if (lw > maxLabelW) maxLabelW = lw;
    if (lh > maxLabelH) maxLabelH = lh;
  }
  ...
  // ponytail: bracket hugs the label itself (maxLabelH + 10) rather than the
  // full row height, so the bottom bracket sits just below the text.
  {
    int16_t sy = startY + (int16_t)selectedIndex * itemH;
    int16_t bracketX = 4;
    int16_t bracketY = sy - 5;
    int16_t bracketW = (35 - bracketX) + (int16_t)maxLabelW + 10;
    int16_t bracketH = (int16_t)maxLabelH + 10;
